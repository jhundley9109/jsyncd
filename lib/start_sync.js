    ...
    applicationFolders.forEach((directoryConfig) => {
      ...
      chokidar.watch(sourcePath, chokidarWatchOptions).on('all', (event, localFileDir) => {
        ...
        if (rsyncExcludePattern.length)
          ...
          // for (let i = 0; i < rsyncExcludePattern.length; i++)
          // {
          //   rsync.patterns([{action: '-', pattern: rsyncExcludePattern[i]}])
          // }
        ...
        rsync.destination(remoteHostUri + destinationPath + relativePathFromSource)
