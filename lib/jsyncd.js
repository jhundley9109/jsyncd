...
class Jsyncd {
    ...
    applicationDirectories.forEach((directoryConfig, directoryIndex) => {
      ...
      chokidar.watch(sourcePath, chokidarWatchOptions).on('all', (event, localFileDir) => {
        ...
        rsync.execute().then((exitCode) => {
          this.sendColoredSyncToLog(`${this.timestampNormalLog()}${appName} Finished with exitcode: ${exitCode}`, chalkColorFunc);
        }).catch((error) => {
  ...
  parseRsyncOutHandle(fileHandle) {
    ...
    let formattedOutput = rsyncOutString.replace(successfulSyncRegex, '').replace(/^/gm, '    ');
