...
class Jsyncd {
  ...
  syncApp(appConfig, appIndex) {
    ...
    if (!Array.isArray(applicationDirectories) || !applicationDirectories.length) {
      let error = new Error('Invalid or empty config.appConfig.directories');
      error.type = 'Misconfigured config file';
      throw error;
    }
    ...
    applicationDirectories.forEach((directoryConfig, directoryIndex) => {
      ...
      const sshObjString = Object.entries(appConfig.sshShellOptions || {}).reduce((sshString, [key, value]) => {
        return `${sshString} ${key} ${value}`;
      }, '');
      ...
      const watcher = chokidar.watch(sourcePath, chokidarWatchOptions).on('all', (event, localFileDir) => {
        ...
        const activeSyncArray = Object.values(activeDirectorySyncs).filter(directorySyncInfo => {return directorySyncInfo.syncing;});
        ...
        let appName = (appConfig.name && ` ${appConfig.name}:[${directoryIndex}]`) || '';
  ...
  buildAndRunRsync(rsyncBuildOptions, chalkColorFunc, appName='') {
    return new Promise(resolve => {
      ...
      if (this._config.debug) {
        this.sendToLog(rsync.command(), chalkColorFunc);
      }
      ...
      rsync.execute().then((exitCode) => {
        this.sendToLog(`${this.getTimestamp()}${appName} Finished with exitcode: ${exitCode}`, chalkColorFunc);
      }).catch((error) => {
  ...
  parseRsyncOutHandle(fileHandle) {
    ...
    let formattedOutput = rsyncOutString.replace(rsyncOutputRegex, '').replace(/^/gm, '    ');
