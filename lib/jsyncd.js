...
class Jsyncd {
  ...
  syncApp(appConfig, appIndex) {
    ...
    // filter out any key/values that are false except 0. And both string and numeric 0 are valid options. JS is stupid in this way.
    const sshObjString = Object.entries(appConfig.sshShellOptions || {}).map(entry => entry.filter(e => String(e) === '0' || e).join(' ')).join(' ');
  ...
  parseRsyncOutHandle(fileHandle) {
    ...
    let formattedOutput = rsyncOutString.replace(rsyncOutputRegex, '').replace(this._rsyncStartOfLineRegex, '    ');
