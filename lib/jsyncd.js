...
class Jsyncd {
  ...
  syncApp(syncConfig) {
    ...
    applicationFolders.forEach((directoryConfig, directoryIndex) => {
      ...
      chokidar.watch(sourcePath, chokidarWatchOptions).on('all', (event) => {
        ...
        this.sendToLog(`${this.getTimestamp()} Calling rsync for ${sourcePath} -> ${destinationPath}`);

        if (this._config.logRsyncCommand) {
          this.sendToLog(rsync.command());
        }

        this.runRsyncExecute(rsync).catch(() => {}).then(() => {
  ...
  runRsyncExecute(rsync) {
    return new Promise((resolve, reject) => {
      ...
      rsync.execute((error, exitCode) => {
        if (error) {
          this.sendToLog(`${this.getTimestamp()} ${error}`);
        ...
        this.sendToLog(`${this.getTimestamp()} ${outputContent}`);
      });
    });
  }
  ...
  parseRsyncOutHandle(fileHandle) {
    ...
    let sucessfulSyncRegex = new RegExp(/^((<f\S*)|(cd\S*)) /gm);

    let rsyncOutString = fileHandle.toString().trim();

    let formattedOutput = rsyncOutString.replace(sucessfulSyncRegex, '');
    return [formattedOutput, rsyncOutString.match(sucessfulSyncRegex)];
  }
