...
class Jsyncd {
  ...
  syncApp(appConfig, appIndex) {
    ...
    if (!Array.isArray(applicationDirectories) || !applicationDirectories.length) {
      throw new ConfigFileError('Invalid or empty config.appConfig.directories');
    }
    ...
    const sshObjString = Object.entries(appConfig.sshShellOptions || {}).map(entry => entry.filter(e => e || String(e) === '0').join(' ')).join(' ');
